isdefined(Main, :ExampleSupport) || include(joinpath(@__DIR__, "..", "..", "ExampleSupport.jl"))
    ...
    module CaseKsliceBerryCurvatureConventional
    ...
    if abspath(PROGRAM_FILE) == abspath(@__FILE__)
        Main.ExampleSupport.run_case(
            CaseKsliceBerryCurvatureConventional.build_config;
            tutorial_dir=normpath(joinpath(@__DIR__, "..")),
            task_id="kslice_berry_curvature_conventional",
            args=ARGS,
            requires_optional=false,
            summary_fields=Dict{String, Any}("calculation" => "K-slice", "mesh" => [200, 200], "component" => [1, 2], "band_selection" => Dict("subspace" => [19, 20]), "photon_energy_ev" => nothing),
        )
    end
