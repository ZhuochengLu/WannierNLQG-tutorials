isdefined(Main, :ExampleSupport) || include(joinpath(@__DIR__, "..", "..", "ExampleSupport.jl"))
    ...
    module CaseKsliceInterbandQuantumMetricConventional
    ...
    if abspath(PROGRAM_FILE) == abspath(@__FILE__)
        Main.ExampleSupport.run_case(
            CaseKsliceInterbandQuantumMetricConventional.build_config;
            tutorial_dir=normpath(joinpath(@__DIR__, "..")),
            task_id="kslice_interband_quantum_metric_conventional",
            args=ARGS,
            requires_optional=false,
            summary_fields=Dict{String, Any}("calculation" => "K-slice", "mesh" => [200, 200], "component" => [2, 2], "band_selection" => Dict("first" => [21, 22], "second" => [19, 20]), "photon_energy_ev" => nothing),
        )
    end
