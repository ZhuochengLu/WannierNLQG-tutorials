isdefined(Main, :ExampleSupport) || include(joinpath(@__DIR__, "..", "..", "ExampleSupport.jl"))
    ...
    module CaseIntegralInjectionCurrentConventional
    ...
    if abspath(PROGRAM_FILE) == abspath(@__FILE__)
        Main.ExampleSupport.run_case(
            CaseIntegralInjectionCurrentConventional.build_config;
            tutorial_dir=normpath(joinpath(@__DIR__, "..")),
            task_id="integral_injection_current_conventional",
            args=ARGS,
            requires_optional=false,
            summary_fields=Dict{String, Any}("calculation" => "Integral", "mesh" => [100, 100, 1], "component_for_plot" => [2, 2, 2], "fermi_energy_ev" => -2.5, "temperature_k" => 0.0),
        )
    end
