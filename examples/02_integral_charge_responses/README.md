# Integral charge responses

Frequency-dependent shift and injection charge-current tensors.
...
|---|---|---|---:|---:|---|---|---|
| `shift_current_conventional.jl` | `shift_current` | `conventional` | 100×100×1 | 0-4 (200 points) | `(2,2,2)` | `"all bands"` | no |
| `shift_current_geometric_loop.jl` | `shift_current` | `geometric_loop` | 100×100×1 | 0-4 (200 points) | `(2,2,2)` | `"all bands"` | no |
| `shift_current_wilson_loop.jl` | `shift_current` | `wilson_loop` | 100×100×1 | 0-4 (200 points) | `(2,2,2)` | `"all bands"` | no |
| `injection_current_conventional.jl` | `injection_current` | `conventional` | 100×100×1 | 0-4 (200 points) | `(2,2,2)` | `"all bands"` | no |
...
```bash
julia --project=. examples/02_integral_charge_responses/run.jl --profile=smoke
julia --project=. examples/02_integral_charge_responses/run.jl --profile=reference
...
```bash
julia --project=. examples/02_integral_charge_responses/cases/shift_current_conventional.jl --profile=smoke
julia --project=. examples/02_integral_charge_responses/cases/shift_current_geometric_loop.jl --profile=smoke
julia --project=. examples/02_integral_charge_responses/cases/shift_current_wilson_loop.jl --profile=smoke
julia --project=. examples/02_integral_charge_responses/cases/injection_current_conventional.jl --profile=smoke
```
...
- `integral_shift_current_conventional`: `GeS_sc_conv.dat`
- `integral_shift_current_geometric_loop`: `GeS_sc_geo.dat`
- `integral_shift_current_wilson_loop`: `GeS_sc_wilson.dat`
- `integral_injection_current_conventional`: `GeS_ic_conv.dat`
