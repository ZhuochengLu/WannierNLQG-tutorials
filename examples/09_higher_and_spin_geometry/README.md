# Higher and spin geometry

Hermitian curvature, triple-phase products, and Zeeman-weighted interband geometry.
...
|---|---|---|---:|---:|---|---|---|
| `hermitian_curvature_tensor_conventional.jl` | `hermitian_curvature_tensor` | `conventional` | 200×200 | None | `(2,2,1,2)` | `{"second":[19,20],"first":[21,22]}` | no |
| `triple_phase_product_conventional.jl` | `triple_phase_product` | `conventional` | 200×200 | None | `(2,2,2)` | `{"second":[23,24],"first":[21,22],"third":[19,20]}` | no |
| `zeeman_interband_berry_curvature_conventional.jl` | `zeeman_interband_berry_curvature` | `conventional` | 200×200 | None | `(1,3)` | `{"second":[19,20],"first":[21,22]}` | yes |
| `zeeman_interband_quantum_metric_conventional.jl` | `zeeman_interband_quantum_metric` | `conventional` | 200×200 | None | `(1,3)` | `{"second":[19,20],"first":[21,22]}` | yes |

HCT selects `(2,2,1,2)` on ordered `C1,V`; TPP selects `(2,2,2)` on ordered `C1,C2,V`; Zeeman IBC/IQM select `(1,3)` on ordered `C1,V`.
...
```bash
julia --project=. examples/09_higher_and_spin_geometry/run.jl --profile=smoke
julia --project=. examples/09_higher_and_spin_geometry/run.jl --profile=reference
...
```bash
julia --project=. examples/09_higher_and_spin_geometry/cases/hermitian_curvature_tensor_conventional.jl --profile=smoke
julia --project=. examples/09_higher_and_spin_geometry/cases/triple_phase_product_conventional.jl --profile=smoke
julia --project=. examples/09_higher_and_spin_geometry/cases/zeeman_interband_berry_curvature_conventional.jl --profile=smoke
julia --project=. examples/09_higher_and_spin_geometry/cases/zeeman_interband_quantum_metric_conventional.jl --profile=smoke
```
...
- `kslice_hermitian_curvature_tensor_conventional`: `GeS_hctk_i_conv.dat`, `GeS_hctk_r_conv.dat`
- `kslice_triple_phase_product_conventional`: `GeS_tppk_i_conv.dat`, `GeS_tppk_r_conv.dat`
- `kslice_zeeman_interband_berry_curvature_conventional`: `GeS_zibck_conv.dat`
- `kslice_zeeman_interband_quantum_metric_conventional`: `GeS_ziqmk_conv.dat`
