isdefined(Main, :ExampleSupport) || include(joinpath(@__DIR__, "..", "..", "ExampleSupport.jl"))
    ...
    module CaseKsliceTriplePhaseProductConventional
    ...
    wannier_center_convention = "Convention_II",
            ...
            tasks = [TaskSpec(
                ...
                observable = KSliceSelection(
                    ...
                    bands=TripleGroups(first=[21, 22], second=[23, 24], third=[19, 20]),
                ),
            )],
    ...
    if abspath(PROGRAM_FILE) == abspath(@__FILE__)
        Main.ExampleSupport.run_case(
            CaseKsliceTriplePhaseProductConventional.build_config;
            tutorial_dir=normpath(joinpath(@__DIR__, "..")),
            task_id="kslice_triple_phase_product_conventional",
            args=ARGS,
            requires_optional=false,
            summary_fields=Dict{String, Any}("calculation" => "K-slice", "mesh" => [200, 200], "component" => [2, 2, 2], "band_selection" => Dict("first" => [21, 22], "second" => [23, 24], "third" => [19, 20]), "photon_energy_ev" => nothing),
        )
    end
